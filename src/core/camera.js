 ...
 */
function detectPoseInRealTime(video, net, onUpdate) {
  ...
  async function poseDetectionFrame() {
    ...
    poses.forEach(({ score, keypoints }) => {
      if (score >= minPoseConfidence) {
        ...
        if (onUpdate) {
          onUpdate({ score, keypoints }, { videoHeight, videoWidth });
        }
      }
    });
 ...
 */
export async function bindPage(onUpdate) {
  toggleLoadingUI(true);
  ...
  setupGui([], net);
  setupFPS();
  detectPoseInRealTime(video, net, onUpdate);
}
